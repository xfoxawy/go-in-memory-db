...
func TestAddRoot(t *testing.T) {
	BinaryTree := addRoot(&Node{0, "alaa1", nil, nil})
...
func TestInsert(t *testing.T) {
	BinaryTree := addRoot(&Node{5, "node", nil, nil}) // root
...
func TestIsLeaf(t *testing.T) {
	testLeafTree := addRoot(&Node{5, "node", nil, nil}) // root
	testLeafNode := testLeafTree.node
	if testLeafNode.isLeaf() != true {
		t.Error("expected", true, "got", false)
	}
	testLeafNode.insert(&Node{9, "node1", nil, nil}) // first right
...
func TestHasLeft(t *testing.T) {
	testHasLeftTree := addRoot(&Node{5, "node", nil, nil}) // root
	testhasLeftNode := testHasLeftTree.node
	if testhasLeftNode.hasLeft() != false {
		t.Error("expected", false, "got", true)
	}
	testhasLeftNode.insert(&Node{3, "node6", nil, nil}) // left for root
...
func TestHasRight(t *testing.T) {
	testHasRightTree := addRoot(&Node{5, "node", nil, nil}) // root
	testhasRightNode := testHasRightTree.node
	if testhasRightNode.hasRight() != false {
		t.Error("expected", false, "got", true)
	}
	testhasRightNode.insert(&Node{9, "node1", nil, nil}) // first right
